...
def main():
    ...
    val = "asdasdasasasasdasdsadasdsadsa"
    ...
    while not gameExit:
        ...
        while gameState == "game" and not gameExit:
            ...
            if uiState == "inventory":
                spriteWorldPos = [int(getWorldPos(mousePos)[0]), int(getWorldPos(mousePos)[1])]
            ...
            if time.time() - lastFrameTime > 0.006:
                for player in range(len(players)):
                    if player != clientPlayerID:
                        players[player]["pos"] = [players[player]["pos"][0] + (time.time() - lastFrameTime) * playerSpeed * players[player]["delta"][0] * 1000, players[player]["pos"][1] + (time.time() - lastFrameTime) * playerSpeed * players[player]["delta"][1] * 1000]
